...
# words. In this example, the second and third sentences (underlined) have the 
...
class Solution:
    def mostWordsFound(self, sentences: List[str]) -> int:
        ...
        for sentence in sentences:
            # count the whitespaces in the string, add 1 because there should be
            # 1 more word than whitespace. check to see if we need to update the max
            num = max(num, sentence.count(' ') + 1)
