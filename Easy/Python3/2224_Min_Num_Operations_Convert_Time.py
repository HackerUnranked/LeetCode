# You are given two strings current and correct representing two 24-hour times.

# 24-hour times are formatted as "HH:MM", where HH is between 00 and 23, and MM 
# is between 00 and 59. The earliest 24-hour time is 00:00, and the latest is 
# 23:59.

# In one operation you can increase the time current by 1, 5, 15, or 60 minutes. 
# You can perform this operation any number of times.

# Return the minimum number of operations needed to convert current to correct.

# Example 1:

# Input: current = "02:30", correct = "04:35"
# Output: 3
# Explanation:
# We can convert current to correct in 3 operations as follows:
# - Add 60 minutes to current. current becomes "03:30".
# - Add 60 minutes to current. current becomes "04:30".
# - Add 5 minutes to current. current becomes "04:35".
# It can be proven that it is not possible to convert current to correct in 
# fewer than 3 operations.

# Example 2:

# Input: current = "11:00", correct = "11:01"
# Output: 1
# Explanation: We only have to add one minute to current, so the minimum number 
# of operations needed is 1.

class Solution:
    def convertTime(self, current: str, correct: str) -> int:
        total = 0
        
        # get the minutes
        cur_min = int(current[3:])
        corr_min = int(correct[3:])
        
        # find the difference
        diff = abs(corr_min - cur_min)
        sub_1 = False # subtract an hour if the current min is greater than the correct min
        
        if corr_min < cur_min:
            diff = 60 - diff # get the diff to rollback to the corr min because it's smaller
            sub_1 = True
        
        # see how many 15 increments we can make
        if diff >= 15:
            total = diff // 15
            diff %= 15 # get the remainder
        
        # see how many 5 increments we can make
        if diff >= 5:
            total += diff // 5
            diff %= 5
            
        # add the remaining minutes
        if diff > 0:
            total += diff
        
        # add the difference in hours
        total += int(correct[0:2]) - int(current[0:2])
        
        # subtract one if the min was smaller in the correct time
        if sub_1 == True:
            total -= 1
                
        return total