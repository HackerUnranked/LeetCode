...
class Solution:
    def finalPrices(self, prices: List[int]) -> List[int]:
        ...
        for idx in range(len(prices) - 1):
            found = False # flag if we found the minimum
            # loop starting at an index 1 ahead and loop until the end
            for idx2 in range(idx + 1, len(prices), 1):
                # if we found a less value break and leave the loop 
                if prices[idx2] <= prices[idx]:
                    found = True # set the flag
                    break
            # we found a lesser value, change it otherwise do nothing to it
            if found == True:
                prices[idx] -= prices[idx2]
