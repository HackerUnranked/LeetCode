...
#     Let n be the length of nums. If n == 1, end the process. Otherwise, create 
#     a new 0-indexed integer array newNums of length n / 2.
#     
...
class Solution:
    def minMaxGame(self, nums: List[int]) -> int:
        ...
        while len(nums) != 1:
            a = []
            count = 0 # keep track of even odd pair of 2
            
            # loop by 2
            for idx in range(0, len(nums), 2):
                # even pair of 2 we take the min
                if count % 2 == 0:
                    a.append(min(nums[idx], nums[idx + 1]))
                # odd we take the max
                else:
                    a.append(max(nums[idx], nums[idx + 1]))
