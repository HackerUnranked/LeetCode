...
class Solution:
    def replaceDigits(self, s: str) -> str:
        a = "" # to create new string
        prev = "" # keeps track of previous character
        
        # loop the string
        for letter in s:
            # concat if it's not a digit and keep track of the added character so we know how to shift
            if letter.isdigit() == False:
                a += letter
                prev = letter
            # shift the character and concat
            else:
                a += chr(ord(prev) + int(letter))
