...
# Input: arr = [6,0,7,0,7,5,7,8,3,4,0,7,8,1,6,8,1,1,2,4,8,1,9,5,4,3,8,5,10,8,6,
#              6,1,0,6,10,8,2,3,4]
# Output: 4.77778
...
class Solution:
    def trimMean(self, arr: List[int]) -> float:
        ...
        for idx1, val1 in enumerate(arr):
            for idx2 in range(0, len(arr) - idx1 - 1):
                if arr[idx2] > arr[idx2 + 1]:
                    temp = arr[idx2]
                    arr[idx2] = arr[idx2 + 1]
                    arr[idx2 + 1] = temp
        
        small = int(len(arr) *.05) # get how many to remove
        big = small # copy the value above because it's the same
        temp = arr[small:len(arr) - big]
