...
class Solution:
    def minBitFlips(self, start: int, goal: int) -> int:
        ...
        # NOTE: This is the same solution as 461 hamming distance
        num = start ^ goal # this gives us all the bits that are different
        count = 0
        # loop and count the ones
        for x in range(32):
            count += num & 1 # if it's a 1 then we know it is different and we count it
            num = num >> 1 # shift right because we just counted one of the bits
