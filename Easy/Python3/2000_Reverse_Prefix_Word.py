...
class Solution:
    def reversePrefix(self, word: str, ch: str) -> str:
        ...
        # loop the string until we find the character
        for idx in range(len(word)):
            # if we found the ch the create the string
            if word[idx] == ch:
                w = word[idx::-1] # slice the string backwards
                w += word[idx+1:] # concat with the rest of the string
                return w # return
        
        return word # we didn't find the string