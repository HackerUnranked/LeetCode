...
class Solution:
    def isSumEqual(self, firstWord: str, secondWord: str, targetWord: str) -> bool:
        ...
        # loop each of the strings and concact the numeric value
        # to build a new string then we add the two strings and compare
        # it with total to see if there is a match
        for letter in firstWord:
            total1 += str(ord(letter) - 97)
        for letter in secondWord:
            total2 += str(ord(letter) - 97)
        for letter in targetWord:
            target += str(ord(letter) - 97)
        
        return True if int(total1) + int(total2) == int(target) else False