...
class Solution:
    def scoreOfString(self, s: str) -> int:
        ...
        for a in range(1,len(s)):
            # subtract current from prev if prev is bigger
            if ord(prev) > ord(s[a]):
                val += ord(prev) - ord(s[a])
            # subtract prev from current if prev is smaller
            elif ord(prev) < ord(s[a]):
                val += ord(s[a]) - ord(prev)
            prev = s[a]
