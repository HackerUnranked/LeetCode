...
#     of a is before the second occurrence of b. s will contain at least one 
#     letter that appears twice. 
...
class Solution:
    def repeatedCharacter(self, s: str) -> str:
        # bucket to count the appearance of each character
        buckets = [0]*26
        
        # loop the string and count it if we haven't seen
        # return the char if we seen it more than once
        for x in s:
            if buckets[ord(x) -97] == 0:
                buckets[ord(x) - 97] += 1
            else:
                return x