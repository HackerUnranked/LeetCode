...
class Solution:
    def countBalls(self, lowLimit: int, highLimit: int) -> int:
        ...
        for i in range(lowLimit, highLimit + 1):
            sm = 0
            # loop and add the digits
            while i > 0:
                
                sm += i % 10 # get the remainder to add
                i //= 10 # divide by 10 because we got the remainder
                
            cnt[sm - 1] += 1 # add the number we seen into the bucket
