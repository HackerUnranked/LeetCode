...
class Solution:
    def evaluateTree(self, root: TreeNode) -> bool:
        ...
        if not root:
            return None
        # if i am a parent then i need to traverse right and left
        if root.left and root.right:
            right = self.evaluateTree(root.right)
            left = self.evaluateTree(root.left)
            
            # OR the values if i am a 2
            if root.val == 2:
                return left | right
            # AND the values if i am a 3
            else:
                return right & left
        else:
            # return my root value because i am a leaf node
            return root.val
