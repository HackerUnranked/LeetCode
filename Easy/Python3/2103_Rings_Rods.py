...
class Solution:
    def countPoints(self, rings: str) -> int:
        # map each rod to a set, defaultdict allows us to map each key to a set obj
        dic = defaultdict(set)
        idx = 0
        
        # loop the string slicing by 2
        while idx < len(rings) - 1:
            # add the pair into the dictionary mapping the ring to the set val
            dic[int(rings[idx+1])].add(rings[idx])
            idx += 2
        
        counts = 0
        
        # loop the dictionary and where the set is of len 3 that means it
        # holds all of the 3 colors
