...
class Solution:
    def truncateSentence(self, s: str, k: int) -> str:
        ...
        for idx, val in enumerate(s):
            # if i reached a space then count it
            if val == ' ':
                num += 1
                # if num space is the same as k the i return
                # the string from 0-k idx because each word is
                # seperated by a space
                if num == k:
                    return s[0:idx]

        # return s in the case we reached the end because it has no space for the last word        
        return s 