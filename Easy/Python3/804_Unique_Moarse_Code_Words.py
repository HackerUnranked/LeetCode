...
class Solution:
    def uniqueMorseRepresentations(self, words: List[str]) -> int:
        collect = set() # set to keep track of the different words
        
        # mapping to the moarse code
        the_key = [".-", "-...", "-.-.", "-..", ".", "..-.", "--.", "....",
                   "..", ".---", "-.-", ".-..", "--", "-.", "---", ".--.",
                   "--.-", ".-.", "...", "-", "..-", "...-", ".--", "-..-",
                   "-.--", "--.."]
        ...
        # loop the words
        for word in words:
            w = "" # a string to build the word
            # loop the letters in the word
            for letter in word:
                # build the string
                w += the_key[ord(letter) - 97]
            # check if the string is in the set, if not then we add it
            if w not in collect:
                collect.add(w)
            
        
        return len(collect) # return the length of the unique count