...
class Solution:
    def processQueries(self, queries: List[int], m: int) -> List[int]:
        ans = [] # answer to store position
        arr = [element for element in range(1, (m+1))] # create array permutation
        
        for i in queries:
            ans.append(arr.index(i)) # append the idx of the ith element
            arr.pop(arr.index(i)) # remove the element at the position from the arr array
            arr.insert(0,i) # add the removed, element to the beginning
