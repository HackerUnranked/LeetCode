...
class Solution:
    def deckRevealedIncreasing(self, deck: List[int]) -> List[int]:
        # sort the array then simulate the process in the instructions
        arr = [False] * max(deck) # we will use bucket sort here
        
        # mark the number as seen if it's in the deck
        for num in deck:
            arr[num - 1] = True
        
        # now we need to simulate and build the array
        queue = deque()
        
        # loop the list backwards, we are reverse engineering the process
        for idx in range(len(arr) - 1, -1, -1):
            # if the number exists in the arr
            if arr[idx] == True:
                # if we have something in the queue then move the last item to the beginning
                if queue:
                    queue.appendleft(queue.pop())
                
                queue.appendleft(idx + 1) # add current number to the top of the deck
