...
class Solution:
    def wateringPlants(self, plants: List[int], capacity: int) -> int:
        temp = capacity # keeps track of how much water we have left
        count = 0  # stores the number of steps
        
        for idx, val in enumerate(plants):
            # we have enough water
            if val <= temp:
                temp -= val # subtract the water
                count += 1 # keep track of how many steps
            # we don't have enough water
            else:
                # we only have to go back once
                if val <= capacity:
                    count += (idx * 2) + 1
                    temp = capacity - val
                # we have to go back more than once
                else:
                    count += (idx * (val // capacity) * 2) + 1 # calculate how many times we have to go back
                    temp = capacity % val # get what is left in the tank
