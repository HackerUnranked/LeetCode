...
class Solution:
    def numberOfBeams(self, bank: List[str]) -> int:
        prev = -1 # prev number of 1s
        total = 0 # total laser we can point
        
        for string in bank:
            count = 0
            for ones in string: # count how many 1s are in the string
                if ones == '1':
                    count += 1
                    
            # get the total lazers we can point
            if prev != -1 and count != 0:
                total += prev * count
            if count != 0:
                prev = count # set prev value
