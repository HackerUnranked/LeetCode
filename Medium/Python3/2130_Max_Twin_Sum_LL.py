...
class Solution:
    def pairSum(self, head: ListNode) -> int:
        total = 0 # keep track of the biggest sum
        arr = [] # store the nodes
        count = 0
        ...
        mid = len(arr) // 2 # get the mid so we know when to stop
        
        # loop while we are not at the mid
        while count != mid:
            val = arr.pop() + arr[count] # get the twin sum
