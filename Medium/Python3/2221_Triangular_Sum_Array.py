...
class Solution:
    def triangularSum(self, nums: List[int]) -> int:
        diff = len(nums) # keeps track if the array section we are adding to
        
        while diff != 0:
            # for loop and add the values adjacent to each other.
            # each iteration the array gets smaller by 1 so we use
            # diff to keep track of the new size
            for idx in range(diff - 1):
                nums[idx] = (nums[idx] + nums[idx + 1]) % 10 # get the difference and mod
            
            diff -= 1 # keeps track of new array size
        
        return nums[0] # returns answer
